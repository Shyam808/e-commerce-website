...
$id = $_GET['id'];
$select = "SELECT * FROM addressone WHERE id='$id'";
$result = mysqli_query($conn, $select);
$row = mysqli_fetch_array($result);

if (isset($_POST['upd_address'])) {
    $name = $_POST['name'];
    $mobile = $_POST['mobile'];
    ...
    $sql = "UPDATE addressone SET name='$name', mobile='$mobile', address='$address', city='$city', street_no='$street_no', state='$state', pincode='$pincode' WHERE id='$id'";
    $result = mysqli_query($conn, $sql);
    if ($result) {
        echo "<script>alert('Address updated successfully'); window.location.href='my_address.php';</script>";
    } else {
                    ...
                    <div class="col-md-8">
                        <div class="card" style="border-radius: 2px; border: none; box-shadow: 0 1px 2px 0 rgba(0,0,0,.2);">
                            <div class="card-body p-4">
                                <h4 style="font-weight: 500; color: #212121; margin-bottom: 20px;">Update Address</h4>

                                <h6 style="font-weight: 500; color: #2874f0; text-transform: uppercase; font-size: 14px; margin-bottom: 16px;">Update Address</h6>

                                <form action="" method="post">
                                    <div class="row">
                                        <div class="col-md-6 form-group">
                                            <input type="text" class="form-control" id="name" name="name" placeholder="Name" value="<?php echo $row['name']; ?>" required style="border-radius: 2px; padding: 12px; height: auto;">
                                        </div>
                                        <div class="col-md-6 form-group">
                                            <input type="number" class="form-control" id="mobile" name="mobile" placeholder="10-digit mobile number" value="<?php echo $row['mobile']; ?>" required style="border-radius: 2px; padding: 12px; height: auto;">
                                        </div>
                                    </div>
                                    <div class="form-group mt-2">
                                        <textarea class="form-control" id="address" name="address" placeholder="Address (Area and Street)" rows="3" required style="border-radius: 2px; padding: 12px;"><?php echo htmlspecialchars($row['address']); ?></textarea>
                                    </div>
                                    <div class="row mt-2">
                                        <div class="col-md-6 form-group">
                                            <input type="text" class="form-control" id="city" name="city" placeholder="City/District/Town" value="<?php echo $row['city']; ?>" required style="border-radius: 2px; padding: 12px; height: auto;">
                                        </div>
                                        <div class="col-md-6 form-group">
                                            <input type="text" class="form-control" id="street_no" name="street_no" placeholder="Street No./Name" value="<?php echo $row['street_no']; ?>" required style="border-radius: 2px; padding: 12px; height: auto;">
                                        </div>
                                    </div>
                                    <div class="row mt-2">
                                        <div class="col-md-6 form-group">
                                            <input type="text" class="form-control" id="state" name="state" placeholder="State" value="<?php echo $row['state']; ?>" required style="border-radius: 2px; padding: 12px; height: auto;">
                                        </div>
                                        <div class="col-md-6 form-group">
                                            <input type="number" class="form-control" id="pincode" name="pincode" placeholder="Pincode" value="<?php echo $row['pincode']; ?>" required style="border-radius: 2px; padding: 12px; height: auto;">
                                        </div>
                                    </div>
                                    <div class="mt-4">
                                        <button type="submit" name="upd_address" class="btn btn-primary" style="background-color: #fb641b; border: none; border-radius: 2px; padding: 12px 36px; font-weight: 500; font-size: 15px; box-shadow: 0 1px 2px 0 rgba(0,0,0,.2);">Update Address</button>
                                        <a href="my_address.php" class="btn btn-link ml-4" style="color: #2874f0; font-weight: 500; text-decoration: none;">CANCEL</a>
                                    </div>
                                </form>
                                <?php //endif; 
                                ?>
